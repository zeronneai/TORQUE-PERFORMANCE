// ============================================================
// TORQUE PERFORMANCE — Set membership status from the owner's list
// READ-THEN-CONFIRM (dry-run by default; --apply writes).
//
// --status inactive (default) — data cleanup ('Delete' list).
// --status canceled           — the owner's real 'Cancelled' list (distinct from
//                               inactive so churn stays separable in the admin).
//
...
//   normalize = lowercase, strip, NBSP/zero-width → space, collapse whitespace.
//   0 or >1 active membership → NOT guessed; skipped and written to ambiguous.csv.
//
// STRIPE GUARDS (never touches Stripe):
//   active/trialing  → SKIP + report loudly (still a paying member).
//   past_due/paused  → SKIP + report separately (still OPEN in Stripe, may keep
//                      charging) → owner must cancel these in the Stripe dashboard.
//   canceled/unpaid/incomplete/unknown, or manual/pi_/null → proceed.
//
// OUTPUTS (always): deactivate-plan.csv, deactivate-rollback.csv (id + prior status),
//   ambiguous.csv, stripe-active-skips.csv, stripe-open-subs.csv.
//
// Run (bash):
//   STRIPE_SECRET_KEY=sk_live_... VITE_SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... \
//     node deactivate-from-list.mjs cancelled-list.csv --status canceled   # dry run
//   ...add --apply to write.
// ============================================================
...
const ACTIVE_STRIPE = new Set(['active', 'trialing']);   // never touch — still a paying member
const OPEN_STRIPE   = new Set(['past_due', 'paused']);   // still OPEN in Stripe → owner must cancel there; we don't mark it

// --status <inactive|canceled> — what matched rows are set to. Default 'inactive'
// (data cleanup). Use --status canceled for the owner's real "Cancelled" list.
const _si = process.argv.indexOf('--status');
const NEW_STATUS = (_si >= 0 ? (process.argv[_si + 1] || '').trim().toLowerCase() : 'inactive');
if (!['inactive', 'canceled'].includes(NEW_STATUS)) {
  console.error(`--status must be 'inactive' or 'canceled' (got "${NEW_STATUS}")`); process.exit(1);
}
...
async function main() {
  ...
  const toDeactivate = [];
  const stripeOpen = [];   // past_due / paused → reported for the owner to cancel in Stripe
  let checked = 0;
  for (const p of plan) {
    ...
    if (pid.startsWith('sub_')) {
      ...
      if (ACTIVE_STRIPE.has(st)) { stripeActive.push({ ...p, stripeStatus: st }); continue; }  // DO NOT touch — active payer
      if (OPEN_STRIPE.has(st))   { stripeOpen.push({ ...p, stripeStatus: st }); continue; }     // still open — owner cancels in Stripe
    }
    toDeactivate.push(p);   // manual / pi_ / null, or a sub_ that's canceled/unpaid/incomplete/unknown in Stripe
  }
  ...
  for (const p of toDeactivate) planRows.push([p.m.id, p.m.kid_name, p.row.parent_name, p.row.parent_email, p.row.package_name, p.m.package_name, p.m.stripe_payment_id, p.m.status, NEW_STATUS]);
  ...
  writeFileSync('stripe-open-subs.csv', '﻿' + [['id', 'kid_name', 'parent_email', 'stripe_payment_id', 'stripe_status'], ...stripeOpen.map(s => [s.m.id, s.m.kid_name, s.row.parent_email, s.m.stripe_payment_id, s.stripeStatus])].map(r => r.map(csvCell).join(',')).join('\n') + '\n');

  // ── Report ──
  console.log('\n' + '='.repeat(104));
  console.log(`SET STATUS 'active' → '${NEW_STATUS}'${APPLY ? '' : '   [DRY RUN — no writes]'}`);
  ...
  for (const p of toDeactivate) {
    ...
    console.log(`• ${p.m.kid_name}  |  ${p.row.parent_name || '—'}  |  ${p.row.parent_email}  |  ${p.m.package_name || '—'}  |  ${p.m.stripe_payment_id || 'null'}  |  ${p.m.status} → ${NEW_STATUS}${pkgWarn}`);
  }
  console.log('='.repeat(104));
  console.log(`Sheet rows:                 ${sheet.length}`);
  console.log(`Matched cleanly & to set '${NEW_STATUS}': ${toDeactivate.length}`);
  console.log(`Ambiguous (0 or >1 match):  ${ambiguous.length}   → ambiguous.csv`);
  console.log(`Skipped — Stripe ACTIVE/trialing (still paying): ${stripeActive.length}   → stripe-active-skips.csv`);
  console.log(`Skipped — Stripe past_due/paused (still OPEN, cancel in Stripe): ${stripeOpen.length}   → stripe-open-subs.csv`);
  if (stripeActive.length) {
    console.log(`\n⚠️  OWNER MARKED THIS LIST BUT STRIPE SAYS ACTIVE — ask before touching these:`);
    for (const s of stripeActive) console.log(`   · ${s.m.kid_name} (${s.row.parent_email}) — ${s.m.stripe_payment_id} — Stripe ${s.stripeStatus}`);
  }
  if (stripeOpen.length) {
    console.log(`\n⚠️  STILL OPEN IN STRIPE (past_due/paused) — these may keep charging. Cancel them in the Stripe dashboard yourself; the script did NOT touch Stripe or the DB for them:`);
    for (const s of stripeOpen) console.log(`   · ${s.m.kid_name} (${s.row.parent_email}) — ${s.m.stripe_payment_id} — Stripe ${s.stripeStatus}`);
  }
  console.log(`\nCSVs written: deactivate-plan.csv, deactivate-rollback.csv, ambiguous.csv, stripe-active-skips.csv, stripe-open-subs.csv`);
  ...
  const ans = await confirm(`\nSet ${toDeactivate.length} membership(s) to status '${NEW_STATUS}'? Type 'yes': `);
  ...
  for (const p of toDeactivate) {
    ...
    else { ok++; console.log(`  ✓ ${p.m.kid_name} → ${NEW_STATUS}`); }
  }
  console.log(`\nDone. ${ok}/${toDeactivate.length} set to '${NEW_STATUS}'.${fails.length ? ` ${fails.length} failed.` : ''}`);
